...
void wsl_bypasser_init_deauth(const wifi_ap_record_t *ap_record, int deauth_interval_seconds) {
    ESP_LOGI(TAG, "Initializing de-auth and running it every %i seconds", deauth_interval_seconds);
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(deauth_timer_handle, deauth_interval_seconds * 1000000));
}
