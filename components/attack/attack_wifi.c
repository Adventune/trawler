...
static void eapolkey_frame_handler(void *args, esp_event_base_t event_base,
  ...
  am_append_status_content(frame->payload, frame->rx_ctrl.sig_len);
...
/**
 * @brief Starts deauth attack. Should not be used directly, use am_* functions
 * instead
 * @details Starts deauth attack, sends deauth packets to the target and sniffs
 * for 4-way handshake.
 *
...
attack_errors_t attack_wifi_start_handshake() {
  ...
  if (am_get_current_wifi_config()->type == DEAUTH) {
